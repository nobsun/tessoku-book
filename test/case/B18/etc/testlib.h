...
#define VERSION "0.9.34-SNAPSHOT+MOD"
...
const char *latestFeatures[] = {
        "eoln() accepts both CRLF and LF in strict mode (modified by tatyam)"
        "random_t now uses not LCGs but mt19937_64 (modified by tatyam)",
...
private:
    ...
    long long nextBits(int bits) {
        if (bits == 0) {
            ...
            if (bits < 0)
                __testlib_fail("random_t::nextBits(int bits): n must not be negative");
            if (bits > 63)
